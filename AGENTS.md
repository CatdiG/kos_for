...
### 4. 화면 기능 7종 체크리스트 전수 재검증 & 수급교집합 골든 스냅샷 검증 (수칙 2-4)
- **화면 기능 7종 체크리스트** (기획/화면 단위 점검 목록 — 수칙 2-1의 "자동 스크립트 21종"과는 별개 목록. 이름이 둘 다 숫자+수급 관련이라 헷갈리기 쉬우니 아래 대조표로 구분할 것):
  ...
  7. 정배열/이격도 추세 배지 (이동평균선 연산)
- **⚠️ 21종 스크립트와의 대조표 (커버리지 확인용)**: 1~5번은 수칙 2-1의 자동 스크립트(그룹 A/B/C/D)가 API 레벨로 이미 커버한다. **6번(카운터카드) · 7번(정배열/이격도 배지)은 `verify_21_checks_regression.js`에 해당 검사가 없다** — 이 2개는 스크립트 실행만으로 검증됐다고 보고하지 말고, 반드시 브라우저에서 화면을 직접 열어 수동/Visual Matching(수칙 2-3)으로 확인한 뒤 보고해야 한다.
...
- **전체/부분 검증 구분**: 배포 직전이거나 핵심 공통 파일(`kisApi.ts`, `batchCollector.ts` 등 3개 이상 기능이 공유하는 파일)을 수정할 때는 화면 기능 7종 체크리스트(수칙 2-4) 전체 검증(자동 스크립트 21종 + 수동 확인 2종)을 수행하며, 단일 UI 컴포넌트나 공유되지 않는 전용 함수만 수정할 때는 해당 영향 범위에 대한 부분 검증만 수행해도 됩니다.
