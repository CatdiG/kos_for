...
자신의 생각(`Thought:`) 단계의 **가장 첫 번째 줄(Line 1)**에서 **아래 Part 1 행동 태도 9대 전 수칙에 대한 [MANDATORY PRE-ACTION AUDIT]**을 반드시 실행해야 한다.
...
### 9. 100% 한국어 프롬프트 및 터미널 명령어 한글 안내 의무화 수칙 (수칙 1-9)
- **도구 호출 파라미터 한글화**: 사용자가 도구 실행(터미널 명령어, 파일 수정 등)의 목적과 내용을 안전하게 확인하고 승인할 수 있도록, **모든 도구 호출 파라미터(`toolSummary`, `toolAction`, `Description`, `Instruction` 등)를 반드시 100% 명확한 한국어로 작성**해야 한다.
- **터미널 명령어 내 한글 안내 의무화**: 사용자가 팝업의 영문 명령어만 보고 불안해하지 않도록, 모든 실행 명령어의 첫머리에 **`Write-Host '=== [안내] 작업 내용 설명 ==='`** 및 한글 주석을 필수로 포함하여 팝업 명령어 창에 한글 설명이 대문짝만하게 보이도록 작성해야 한다.
- **모든 응답 100% 한국어화**: 모든 답변, 대화, 보고서, 안내 문구 역시 100% 한국어로 작성하여, 영문 요약으로 인해 위험한 명령어를 무심코 승인하는 사고를 원천 차단한다.
...
### 1. 자동 스크립트 21종 통합 전수 회귀 검증 및 RAW 터미널 원본 로그 무편집 제출 수칙 (수칙 2-1)
- 코드를 수정하거나 완료 보고를 하기 전, 반드시 `node scratch/verify_21_checks_regression.js`를 실행해야 한다. 이 스크립트가 실제로 검증하는 항목은 정확히 아래 4개 그룹, 총 21종이다(그룹명·개수는 스크립트 원문 기준):
  - **그룹 A. 메인 수급 탭 8종** (①외국인 ②기관 ③프로그램 ④급등주-등락률 ⑤단타 종합랭킹-SCORE ⑥당일교집합 ⑦2일연속교집합 ⑧3일연속교집합)
  - **그룹 B. 급등주 서브모드 4종** (등락률·거래량·거래대금·급등주교집합)
  - **그룹 C. 단타 가중치 프리셋 6종** (기본/등락률100%/거래대금100%/수급50·50/커스텀/모멘텀3지표)
  - **그룹 D. 시장격리·신용필터 3종** (KOSPI 격리·KOSDAQ 격리·신용3상태 필터)
  - 8+4+6+3 = **21종**. (수칙 2-4의 "8대 체크리스트"와는 이름만 겹치는 별개 목록이니 혼동 금지 — 아래 수칙 2-4 하단 대조표 참고)
...
- 단 1개의 항목이라도 FAIL이 뜨거나 에러를 삼키는 꼼수(Swallowing) 코드가 발견되면 즉시 작업을 원상복구하고 오류부터 바로잡아야 한다.
...
### 4. 화면 기능 8종 체크리스트 전수 재검증 & 수급교집합 골든 스냅샷 검증 (수칙 2-4)
- **화면 기능 8종 체크리스트** (기획/화면 단위 점검 목록 — 수칙 2-1의 "자동 스크립트 21종"과는 별개 목록. 이름이 둘 다 숫자+수급 관련이라 헷갈리기 쉬우니 아래 대조표로 구분할 것):
  1. 매매순위 3종 (외국인·기관·프로그램)
  2. 수급교집합 (500ms 이내 중복 배지)
  3. 급등주 순위 (등락률·거래량·거래대금 3서브모드, 60초 자동갱신)
  4. 단타 종합랭킹 (종합 점수 연산, 슬라이더 가중치, 7개 상세 지표)
  5. 신용가능 3-상태 (가능, 불가, 미확인)
  6. 상단 3개 카운터 카드 (외국인, 기관, 프로그램 일관된 기준일)
  7. 탐색범위 제한 (10·20·30·50)
  8. 정배열/이격도 추세 배지 (이동평균선 연산)
- **⚠️ 21종 스크립트와의 대조표 (커버리지 확인용)**: 1~5번은 수칙 2-1의 자동 스크립트(그룹 A/B/C/D)가 API 레벨로 이미 커버한다. **6번(카운터카드) · 7번(탐색범위) · 8번(정배열/이격도 배지)은 `verify_21_checks_regression.js`에 해당 검사가 없다** — 이 3개는 스크립트 실행만으로 검증됐다고 보고하지 말고, 반드시 브라우저에서 화면을 직접 열어 수동/Visual Matching(수칙 2-3)으로 확인한 뒤 보고해야 한다.
...
- **전체/부분 검증 구분**: 배포 직전이거나 핵심 공통 파일(`kisApi.ts`, `batchCollector.ts` 등 3개 이상 기능이 공유하는 파일)을 수정할 때는 화면 기능 8종 체크리스트(수칙 2-4) 전체 검증(자동 스크립트 21종 + 수동 확인 3종)을 수행하며, 단일 UI 컴포넌트나 공유되지 않는 전용 함수만 수정할 때는 해당 영향 범위에 대한 부분 검증만 수행해도 됩니다.
