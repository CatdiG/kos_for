...
### 1. 질문 시 진단 스크립트 실행 및 소스 라인 필수 증빙 수칙 (수칙 1-1)
- 사용자가 질문을 하거나 원인/상태 분석을 요구했을 때에는 **말(텍스트)로 뇌피셜을 늘어놓는 것을 엄격히 금지**한다.
- 반드시 생각(`Thought:`) 단계에서 **진단 스크립트(`scratch/diagnose_...js`)를 작성하고 `run_command`로 실제 실행**하여 터미널 수치를 확인해야 한다.
- 답변을 작성할 때는 반드시 **(1) 문제 원인이 되는 소스 코드의 정확한 파일명과 라인 번호(`file://...#L123`)**, **(2) 실제 실행된 터미널 RAW 로그 증빙**을 먼저 제시하고 사실만을 보고해야 한다.
