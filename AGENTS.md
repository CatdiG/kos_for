...
### 1. 21대 전 항목 통합 전수 회귀 검증 및 RAW 터미널 원본 로그 무편집 제출 수칙 (수칙 2-1)
- 코드를 수정하거나 완료 보고를 하기 전, 반드시 `node scratch/verify_21_checks_regression.js`를 실행해야 한다.
- 메인 7대 수급 탭(외인·기관·프로그램·급등주·당일교집합·2일연속·3일연속), 급등주 4대 서브모드, 단타 가중치 슬라이더/단일신호 6종, 시장 격리 필터(KOSPI/KOSDAQ) 및 신용 3-상태 등 21개 전 항목의 응답 속도(ms)와 데이터 유효성을 전수 검증한다.
...
### 4. 8대 핵심 항목 전수 재검증 & 수급교집합 골든 스냅샷 검증 (수칙 2-4)
- **8대 핵심 항목 전수 점검**:
...
- **전체/부분 검증 구분**: 배포 직전이거나 핵심 공통 파일(`kisApi.ts`, `batchCollector.ts` 등 3개 이상 기능이 공유하는 파일)을 수정할 때는 8대 핵심 항목 전체 검증을 수행하며, 단일 UI 컴포넌트나 공유되지 않는 전용 함수만 수정할 때는 해당 영향 범위에 대한 부분 검증만 수행해도 됩니다.
