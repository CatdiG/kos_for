 ...
 */
// 🚨 [버그 수정 - RankingStockDetailChart.tsx와 동일 결함(수칙 1-6)] isFallback 없이 [0,100] 반환
// 여부만으로 "데이터 없음"을 판별하면, 지수 변동폭이 넓은 구간에서 nice-number step이 rawMin보다
// 커져 축 하한이 "정상적으로" 0에 반올림되는 경우까지 진짜 데이터 없음으로 오판해 매물대가 숨는다.
    ...
    <div className="relative bg-white dark:bg-[#131722] border border-slate-200 dark:border-[#2a2e39] rounded-2xl p-3 sm:p-4 shadow-xl transition-all duration-300">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-rows-[auto_auto] gap-2 border border-slate-100 dark:border-[#1e222d] rounded-xl overflow-hidden">
          ...
          <div className="p-2 relative">
            <ResponsiveContainer width="100%" height={PRICE_CHART_CONFIG.containerHeight}>
              <ComposedChart syncId="index-detail-chart" data={displayTrend} margin={PRICE_CHART_CONFIG.margin}>
                ...
                <YAxis stroke={axisColor} tickFormatter={formatYPrice} tick={{ fontSize: 10 }} width={60} domain={priceDomain} ticks={priceTicks} allowDataOverflow={true} />
          ...
          {/* 일간 이격선 범례 바 (종목 상세 차트 1558~1586번 줄과 동일한 방식으로 차트 바깥에 색상/기간 명시) */}
          {showDisparate && (
            <div className="flex items-center justify-center gap-3 pb-1.5 px-2 text-[10px] font-semibold text-slate-600 dark:text-slate-300 shrink-0 flex-wrap">
