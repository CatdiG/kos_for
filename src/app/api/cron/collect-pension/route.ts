...
async function handleCronBatch(request: NextRequest) {
  ...
  if (cronSecret && cronSecret.trim() !== '') {
    ...
    if (!isHeaderValid && !isParamValid) {
      console.warn('[Cron Auth Rejected] /api/cron/collect-pension: CRON_SECRET 인증 실패');
      return NextResponse.json({ error: 'Unauthorized: Invalid CRON_SECRET' }, { status: 401 });
    }
  }

  const startTime = Date.now();
  console.log('[CRON START /api/cron/collect-pension] TOP_300_STOCKS 연기금 매매 정기 수집 시작');

  try {
    // force = true 로 연기금 강제 수집 및 디스크/인메모리 캐시 갱신 실행
    const success = await runTop50BatchCollector(true, 'batch_pension');
    const elapsedMs = Date.now() - startTime;

    const rankingData = await getBatchRankingDataAsync('pension', 'buy', '1d', 'ALL');
    const count = rankingData?.list ? rankingData.list.length : 0;

    console.log(`[CRON COMPLETE /api/cron/collect-pension] 성공: ${success}, 수집종목수: ${count}, 소요시간: ${elapsedMs}ms`);
    ...
    const elapsedMs = Date.now() - startTime;
    console.error(`[CRON ERROR /api/cron/collect-pension] ${elapsedMs}ms 실패:`, error);
    return NextResponse.json(
      { error: error?.message || '연기금 정기 수집 중 오류 발생' },
      { status: 500 }
    );
  }
}
