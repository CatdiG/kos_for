...
import { fetchKisForeignInstitutionRanking, getKisAccessToken } from '../src/lib/kisApi';
...
async function verifyAllRankingRawData() {
  ...
  async function fetchRawInvestorData(symbol: string) {
  ...
  const testCases: Array<{ type: 'foreign' | 'organ'; direction: 'buy' | 'sell'; label: string; limit: number }> = [
  ...
  for (const testCase of testCases) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      console.log(
        `| ${String(item.rank).padStart(2)}위 | ${item.symbol} | ${item.name.padEnd(10)} | ${item.netBuyAmt.toLocaleString()} 백만원 | ${item.netBuyAmtEok} 억원 | ${rawPbmn.toLocaleString()} 백만원 | ${rawEok} 억원 | ${statusTag} |`
      );
