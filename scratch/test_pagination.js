...
async function testPagination() {
  console.log('--- Testing Pagination & Count Params ---');
  
  // Test FID_INPUT_CNT_1 = 0, 30, 50, 100
  for (const cnt of ['0', '30', '50', '100']) {
    const p = `/uapi/domestic-stock/v1/ranking/fluctuation?FID_COND_MRKT_DIV_CODE=J&FID_COND_SCR_DIV_CODE=20170&FID_INPUT_ISCD=0000&FID_RANK_SORT_CLS_CODE=0&FID_PRC_CLS_CODE=0&FID_INPUT_PRICE_1=0&FID_INPUT_PRICE_2=0&FID_VOL_CNT=0&FID_TRGT_CLS_CODE=0&FID_TRGT_EXLS_CLS_CODE=0&FID_DIV_CLS_CODE=0&FID_INPUT_CNT_1=${cnt}&FID_RSFL_RATE1=0&FID_RSFL_RATE2=0`;
    const res = await testApi('FHPST01700000', p);
    const list = res.json?.output || [];
    console.log(`FID_INPUT_CNT_1=${cnt}: returned ${list.length} items. msg: ${res.json?.msg1}`);
    if (list.length > 0) {
      console.log(`  First item: [${list[0].stck_shrn_iscd}] ${list[0].hts_kor_isnm}, Last item: [${list[list.length-1].stck_shrn_iscd}] ${list[list.length-1].hts_kor_isnm}`);
    }
    await sleep(1500);
  }
}
